...
static int inspect_container_helper(const char *id, int timeout, char **container_json, bool with_size_info)
{
    int ret = 0;
...
static int container_inspect_cb(const container_inspect_request *request, container_inspect_response **response)
{
    ...
    *response = util_common_calloc_s(sizeof(container_inspect_response));
    ...
    if (inspect_container_helper(name, timeout, &container_json, request->bsize) != 0) {
        cc = ISULAD_ERR_EXEC;
    }
