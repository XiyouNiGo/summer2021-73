...
+ 存储驱动在操作系统对应的位置应该在哪？
+ thin pool是什么？
+ device mapper为什么需要basic device。
+ VFS这种驱动为什么慢？（不支持写时复制）
+ overlay每层都是硬链接，那我写的代码需要统计每一层的大小，那总数岂不是指数增长了吗？（不是的，因为它只有两层）
+ overlay既然使用的是硬链接，那为什么还会消耗inode。（外层目录是新建目录）
+ overlay实际上通过trusted.overlay.opaque=y来确定是否是opaque的。
+ A directory is made opaque by setting the xattr “trusted.overlay.opaque”
   to “y”. Where the upper filesystem contains an opaque directory, any
   directory in the lower filesystem with the same name is ignored.
+ git rebase -i的edit真香。
+ umount系统调用总是出现Device or resource busy。（需要先close对应的文件描述符）
+ lgetxattr系统调用总是error。
+ 为什么说xfs没有启用d_type时使用docker会出错。
+ 我的wsl似乎支持不了device mapper，我应该怎么测试。
+ quota是什么（磁盘配额）。
+ 在我的机子上，Udev Sync Supported: false，这使我使用不了device mapper。
+ device mapper网上资料真的很少，显然它不是按层划分，而是按设备，不知道怎么做。
+ dm驱动中容器层的内容到底是在哪，挂载dm设备后只看到完整的rootfs，在/var/lib/docker的其他目录也没有看到。
...
+ 为什么需要NaiveDiffDriver这层封装。（应该是实现了一个继承）
+ 计算目录大小时需要对硬链接文件进行重复计算吗（不需要，维护map来去重）
+ 学习一下k8s、容器网络、Linux Namespace。k8s实在没兴趣，还是算了。。
+ NaiveDiffDriver应该是类似一个虚基类，匿名接口ProtoDriver中的方法类似虚方法，NaiveDiffDriver自己也实现了一些方法。
+ ProtoDriver只是一个原型，要求Driver实现里面的方法，实际使用的还是Naive的那四个方法，不使用Driver的四个方法，因为它用着是:d.naiveDiff.DiffSize，而不是d.naiveDiff.ProtoDriver.DiffSize，这里也压根不是继承，匿名结构体才算是继承，这只是用匿名接口规定传入的Driver需要有那些方法。
+ 假如我一个Ubuntu镜像分两层，我创建了一个Ubuntu容器，那么我拥有镜像层1、镜像层2、容器层。SizeRw就是完整容器层merged大小 - 完整镜像层2的merged小。这个减号就是Changes工具类做的事（不能直接算出来减，得判断inode和设备号）。如果直接减，对于覆盖的文件是错误的。假如我镜像层有一个非常大的文件，但是容器层用一个很小的同名文件遮住它，那么我一减其实是一个负数。
+ 用c语言复写go的工具类真是很有挑战的一件事，首当其冲的就是数据结构。
+ 既然写起来这么麻烦，有没有办法用extern "c"，写出c++代码来（不行，即使行，也只能在c++中用c的东西）
+ 新增的文件并不会被编译，如何修改Cmakelist.txt达到这点。
+ Since kernel 2.6.24, the kernel supports associating capability sets with an executable file using  setcap(8).   The
   file  capability  sets are stored in an extended attribute (see setxattr(2) and xattr(7)) named security.capability.
+ 我仿照docker v20.10.7来写代码。

+ Docker 1.10 and later no longer matches image layer IDs with directory names in `/var/lib/docker`. However, there are two key directories. The `/var/lib/docker/devicemapper/mnt` directory contains the mount points for image and container layers. The `/var/lib/docker/devicemapper/metadata`directory contains one file for every image layer and container snapshot. The files contain metadata about each snapshot in JSON format
